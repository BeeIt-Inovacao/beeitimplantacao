...
cmd_start() {
  ...
  if port_in_use "$PORT_SB_API"; then
    ...
    if supabase start --workdir "$SCRIPT_DIR" 2>&1 | tee -a "${LOG_DIR}/supabase-start.log"; then
  ...
  if port_in_use "$PORT_DEV"; then
    ...
    nohup node "${SCRIPT_DIR}/scripts/dev-server.js" \
      >> "$LOG_DEV" 2>&1 &
    echo $! > "$PID_DEV"
    sleep 1
    if port_in_use "$PORT_DEV"; then
...
cmd_stop() {
  ...
  if pid_alive "$PID_DEV"; then
    kill "$(cat "$PID_DEV")" 2>/dev/null && ok "Dev Server encerrado" || warn "Dev Server: falha ao encerrar"
    rm -f "$PID_DEV"
  elif port_in_use "$PORT_DEV"; then
    lsof -ti tcp:"$PORT_DEV" | xargs kill 2>/dev/null && ok "Dev Server encerrado (por porta)" || true
  else
  ...
  # ── Edge Function ────────────────────────────────────────────────────────
  if pid_alive "$PID_EDGE"; then
    kill "$(cat "$PID_EDGE")" 2>/dev/null && ok "Edge Function encerrada" || warn "Edge Function: falha ao encerrar"
    rm -f "$PID_EDGE"
  else
  ...
  if port_in_use "$PORT_SB_API"; then
    ...
    if supabase stop --workdir "$SCRIPT_DIR" 2>&1 | tee -a "${LOG_DIR}/supabase-stop.log"; then
      ok "Supabase parado"
    else
