...
| **IP-1** | Isolamento de Propriedade Intelectual — Motor de dicionários extraído do monólito inseguro para `src/core/bda-dict-engine.js` (493 linhas, IIFE encapsulado). Funções migradas: `fetchDictBlueprint`, `openDictDrawer`, `closeDictDrawer`, `_dictDrawerFilter`, `_dictDrawerCopyJson`, `_dictDrawerToggleRules`, `checkDictSnapshots`, `_refreshDictBadges` + helpers privados (`_beeitTenantIdFromJWT`, `_sha256hex`, `_DICT_TRACKED_ALIASES`, renderização DOM completa do Swagger-Drawer). Monólito reduzido em ~486 linhas (~25.7 KB). Build `node scripts/build-modules.js` passa · `node --check` ✅ · URLs via `window.BEEIT_EDGE_URL` (sem hardcode). Botões DOM legados do cliente permanecem intocados no HTML. | ✅ **CONCLUÍDA (2026-04-29)** | 🟢 |
| **GK-007** | Scanner passivo de comentários HTML no Gatekeeper — `RuleHtmlCommentsDisclosure` adicionada ao `Governanca/app/scanner.py` (v3.1.0-beeit). Varre recursivamente `*.html` excluindo `Governanca/`; ignora marcadores legais `<!--!`; emite Finding LOW/CWE-615 com inventário completo (preview, arquivo, linha). Build NÃO remove comentários — decisão estratégica de OpSec: mapeamento passivo, não supressão. Integrada ao `run_forensic_scan()`. | ✅ **CONCLUÍDA (2026-04-29)** | 🟢 |
| **GRC-001** | Auditoria forense do fluxo legado SPED e certificação de isolamento da PI (Motor de Dicionários). Laudo técnico emitido em `Governanca/laudo_debitos_sped_legacy.md` (privado). **6 débitos técnicos mapeados:** DT-001 parsing síncrono main thread (Alta), DT-002 enrich() pseudo-async (Média), DT-003 rndP() acoplamento filtro↔render + limite 200 silencioso (Alta), DT-004 heap acumulada sem liberação de RAW[] (Média), DT-005 ausência de feedback granular (Baixa), DT-006 globais SPED sem encapsulamento (Média). **4 certificações de isolamento emitidas (ISO-001 a ISO-004):** grep confirma zero contato entre `bda-dict-engine.js` e variáveis SPED (`PARTS`, `EP`, etc.) e vice-versa. Painel GRC (`Governanca/bdapowered.html`) refatorado para layout de 3 colunas (Linha do Tempo · Laudo Imutável Read-Only · Métricas de Risco LGPD) + botão "Imprimir Laudo Executivo" com modal de linguagem de negócio. Classificação de risco: **Alto Risco Operacional**. | ✅ **CONCLUÍDA (2026-04-30)** | 🟠→🟡 |
